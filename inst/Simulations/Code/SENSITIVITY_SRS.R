...
#           when we have a selected sample from one stratum and one cluster.
#
...
run_one_sim <- function(sim_i, pop_dat, forms) {
  ...
  des_wtd_a <- svydesign(ids = ~ 1, strata = NULL,
  ...
  des_04 <- svydesign(ids = ~ 1, strata = NULL,

    weights = 1 / samp$pS, data = samp)
  ...
  des_06 <- svydesign(ids = ~ 1, strata = NULL,

    weights = samp$comb_wt, data = samp)
  ...
  des_07 <- svydesign(ids = ~ 1, strata = NULL,

    weights = samp$wtd_comb_wt, data = samp)
