...
#
#  PURPOSE: Main simulation evaluate the performance of the proposed methods
#           against standard analytic methods and the oracle estimator.
#
...
build_forms <- function(pop_dat) {
  ...
  # form_s <- as.formula(paste0("pS ~ A + ",        # Uncomment for Sensitivity
  #
  #   paste(c(x_cols, u_cols), collapse = " + ")))

  form_s <- as.formula("pS ~ 1")                    # Comment for Sensitivity
...
#--- UNCOMMENT TO SAVE SENSITIVITY ANALYSIS RESULTS ----------------------------

# save(results, file = here("inst", "Simulations", "Results",
#
#   "SENSITIVITY_SUNKNOWN.RData"))
